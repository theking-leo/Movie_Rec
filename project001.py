...
# Load data
df1 = pd.read_csv(
    "https://raw.githubusercontent.com/hanaamulic/WBSFLIX-recommendations/main/data/ml-latest-small/movies.csv")
df2 = pd.read_csv(
    "https://raw.githubusercontent.com/hanaamulic/WBSFLIX-recommendations/main/data/ml-latest-small/ratings.csv")
...
def popular_movie_recommender(top_n=5):
    ...
    recommended_movies = df1.merge(
        movie_rating_counts, on='movieId', how='inner')
    recommended_movies = recommended_movies.sort_values(
        by='rating_count', ascending=False).head(top_n)
    recommended_movies['poster_url'] = recommended_movies['title'].apply(
        ...
        if not recommended_movies.empty:
            ...
            for _, movie in recommended_movies.iterrows():
                if 'poster_url' in movie and movie['poster_url']:
                    ...
                    carousel.image(
                        movie['poster_url'], caption=movie['title'], use_column_width=True)
                    st.markdown(f"**Title**: {movie['title']}")
                    st.markdown(f"**Genres**: {movie['genres']}")
                    st.markdown("---")  # Add a horizontal line
                else:
                    st.markdown(
                        f"**Title**: {movie['title']} (No image available)")
                    st.markdown(f"**Genres**: {movie['genres']}")
...
def collaborative_filtering_recommender(user_id, n=5):
    ...
    movie_ids = [str(movie_id)
                 for movie_id in range(1, df1['movieId'].max() + 1)]
    user_ratings = [(movie_id, model.predict(user_id, movie_id).est)
                    for movie_id in movie_ids]
    user_ratings.sort(key=lambda x: x[1], reverse=True)
    top_n_movies = user_ratings[:n]

    # Get movie poster URLs
    top_n_df = pd.DataFrame(top_n_movies, columns=[
                            'movieId', 'predicted_rating'])
    top_n_df['movieId'] = top_n_df['movieId'].astype(int)
    recommended_movies = pd.merge(
        top_n_df, df1[['movieId', 'title']], on='movieId', how='left')
    recommended_movies['poster_url'] = recommended_movies['title'].apply(
        get_movie_poster_url)
...
def main_user_based_movie():
    ...
    if st.button('Get Recommendations', key="user_rec_button"):
        recommended_movies = collaborative_filtering_recommender(
        ...
        if recommended_movies is not None and not recommended_movies.empty:
            ...
            for _, movie in recommended_movies.iterrows():
                ...
                with cols[1]:  # Second column for movie details
                    ...
                    st.write(
                        f"Predicted Rating: {rating_to_stars(movie['predicted_rating'])}")
        else:
...
def collaborative_filtering_recommender(user_id, n=5):
    ...
    similar_movies = pd.Series(
        item_similarity[user_id], index=user_item_matrix.columns)
    similar_movies = similar_movies[~similar_movies.index.isin(
        rated_movie_ids)]
...
def main():
    ...
    app_mode = st.sidebar.selectbox("Choose the Recommender", [
                                    "Popular Movies", "User-Based", "User ID"])
